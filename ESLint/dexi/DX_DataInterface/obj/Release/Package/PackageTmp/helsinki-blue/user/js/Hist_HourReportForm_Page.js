...
function SelectAndGetDeviceHistData(ST, NAME, StartTime, EndTime) {
    try {
        ...
        if (g_DataAllCnt > 0) {
            g_HistDataObj = ajax_sync_get_hist_data(ST, StartTime, EndTime, false, 0, 100); // 获取第一页数据
        } else {
            g_HistDataObj = [];
        }
        // 获取当前设备的报警配置
...
function table_limt_jump_event(obj, first) {
    try {
        ...
        if (!first) {
            ...
            g_HistDataObj = ajax_sync_get_hist_data(g_ThisST, g_StartTime, g_EndTime, false, g_SelectPageIndex * cg_OnePageDataCount, cg_OnePageDataCount); // 获取第n页数据
...
function DownAllData() {
    ...
    try {
        for (var i = 0; i < page_count; i++) {
            var DataObj = ajax_sync_get_hist_data(g_ThisST, g_StartTime, g_EndTime, false, i * 500, 500); // 获取第一页数据
