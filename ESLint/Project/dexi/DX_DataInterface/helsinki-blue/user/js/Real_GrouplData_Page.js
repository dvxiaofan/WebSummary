var g_ThisGroupDeviceList;                              //当前所选分组的设备列表
var g_SearchDeviceList;                               //符合搜索条件的设备信息，链接到g_ThisGroupDeviceList
...
var g_ThisSelectGroupName;                          //当前选择的分组名称
var g_MyTree;
//表头
var g_table_cols = [];
...
window.onload = function () {   //要执行的js代码段  
    ...
    var AllDeviceList = ajaxSyncGetDeviceList();          //获取当前用户所有设备基本信息列表   
    g_AllGroupList = ajaxSyncGetAllGroupList();        //获取当前用户所有分组列表
    //alert(JSON.stringify(g_AllGroupList), 4);
    g_GroupDeviceList = marshaling_group_device_data(AllDeviceList, g_AllGroupList);//将分组与设备信息进行整合，格式为[{GROUP:'';DEVICE:[{ST,NAME}]}];
    //alert(JSON.stringify(g_GroupDeviceList, 4));          //调试显示信息
    //准备要显示的数据
    var ObjArr = conversion_data_tree_node(g_GroupDeviceList, 0);//子列表数据源
    ...
    if (g_GroupDeviceList == null || g_GroupDeviceList.length == 0) //无数据
    {
        g_ThisGroupDeviceList = [];
        g_ThisSelectGroupName = null; //当前选择的分组无效
    }
    else {
        g_ThisSelectGroupName = g_GroupDeviceList[0].GROUP;     //默认选择的分组名称
        g_ThisGroupDeviceList = g_GroupDeviceList[0].DEVICE;    //选择当前分组数据
    }
    g_DeviceRealData = ajax_sync_get_real_data(g_ThisGroupDeviceList, 0, cg_OnePageDataCount);             //获取第一页要显示的实时数据

    //document.getElementById('lable_all_count_id').innerHTML = '设备总数：' + g_ThisGroupDeviceList.length; //显示所有设备数量
    ...
    data_table_init(g_DeviceInfoDataPage, g_ThisGroupDeviceList.length, cg_OnePageDataCount, 0);  //初始化表格，并显示数据
...
//刷新某个分组的实时数据
function SelectAndGetGroupRealData(index) {
    try {

        //alert('选择的索引：' + index);
        //alert(JSON.stringify(g_GroupDeviceList, 4));
        //下载当前设备的历史数据
        if (g_GroupDeviceList == null || g_GroupDeviceList.length == 0 || index == null) //无数据
        {
            g_ThisGroupDeviceList = [];     //当前选择的设备列表无效
            g_ThisSelectGroupName = null; //当前选择的分组无效
        }
        else {
            g_ThisSelectGroupName = g_GroupDeviceList[index].GROUP;     //默认选择的分组名称
            g_ThisGroupDeviceList = g_GroupDeviceList[index].DEVICE;    //选择当前分组数据
        }
        g_DeviceRealData = ajax_sync_get_real_data(g_ThisGroupDeviceList, 0, cg_OnePageDataCount);             //获取第一页要显示的实时数据
        g_DeviceInfoDataPage = conversion_data_layui_table(g_DeviceRealData, g_ThisPageDeviceList, 1);                   //将获取的数据转换为layui table能显示的数据
        g_table_config.cols[0] = g_table_cols;                                                  //更新字段
...
function SelectionEvent(index, data_name, text) {
    ...
    document.getElementById("cancel_search_button_id").disabled = "true";    //禁用取消搜索按钮
    SelectAndGetGroupRealData(index);   //获取并显示当前选择的分组数据
...
//将分组与设备信息进行整合，格式为[{GROUP:'';DEVICE:[{ST,NAME}]};INDEX:xxx];    
function marshaling_group_device_data(AllDeviceList, AllGroupList) {
    var ArrObj = [];
    var Count = 0;
    ...
    try {
        ...
        GroupObj.GROUP = '默认分组';
        GroupObj.DEVICE = [];
        GroupObj.INDEX = 0; //记录索引
        ArrObj[Count++] = GroupObj;

        // 没有分组
        if (AllGroupList == undefined) return;

        //循环处理分组
        for (var i = 0; i < AllGroupList.length; i++) {
            var GroupObj = new Object();
            GroupObj.GROUP = AllGroupList[i].GROUP;
            GroupObj.DEVICE = [];
            GroupObj.INDEX = 1 + i;
            ArrObj[Count++] = GroupObj;
        }
        //循环处理设备，将设备信息写入到指定的分组中
        for (var j = 0; j < AllDeviceList.length; j++) {
            var index = FindDeviceGroupIndex(AllGroupList, AllDeviceList[j].GROUP);
            ArrObj[index].DEVICE.push(AllDeviceList[j]);
        }
    } catch (e) {
        layer.alert("错误：" + e.message, { icon: 5, scrollbar: false }); //5：失败；6：成功
    }
...
function table_limt_jump_event(obj, first) {
    try {
        ...
        if (!first) {
            ...
            if (g_ThisGroupDeviceList.length == 0) {
            ...
            {
                g_DeviceRealData = ajax_sync_get_real_data(g_ThisGroupDeviceList, g_SelectPageIndex * cg_OnePageDataCount, cg_OnePageDataCount);  //获取第n页要显示的设备详细信息
            }
            else //处于搜索模式
            {
...
function search_name_onclick() {
    var Keyword = document.getElementById("search_device_id").value; //获取搜索框内容
    ...
    g_SearchDeviceList = search_user_list(g_ThisGroupDeviceList, Keyword);    //搜索
    ...
    try {
        ...
        data_table_init(g_DeviceInfoDataPage, g_SearchDeviceList.length, cg_OnePageDataCount, 0);  //初始化表格，并显示数据
...
function search_cancel_onclick() {
    try {
        ...
        loading_message('加载数据中...');            //弹出提示框
        g_DeviceRealData = ajax_sync_get_real_data(g_ThisGroupDeviceList, 0, cg_OnePageDataCount);        //获取第n页要显示的设备详细信息
...
function ajax_sync_get_real_data(DeviceListObj, StartIndex, Count) {
    ...
    try {
        if (DeviceListObj == null || DeviceListObj.length == 0 || Count == 0) {
            layer.msg("要获取的设备列表为空！", { icon: 5, scrollbar: false });      //5：失败；6：成功
